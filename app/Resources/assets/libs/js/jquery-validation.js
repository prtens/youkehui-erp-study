...
$.validator.setDefaults({
  ...
  highlight: function(element, errorClass, validClass) {
    let $inputWrapper = $(element).parentsUntil('.form-group').last();
    let $inputLabel = $inputWrapper.prev('label');
    $inputWrapper.addClass('has-error');
    $inputLabel.addClass('has-error');
    $inputWrapper.find('.help-block').each(function() {
      let $this = $(this);
  ...
  unhighlight: function(element, errorClass, validClass) {
    ...
    $inputLabel.removeClass('has-error');
    $inputWrapper.find('.help-block').each(function() {
      let $this = $(this);
      if (!$this.hasClass('jq-validate-error')) {
        $this.show();
...
}, jQuery.validator.format("请输入中文字、英文字母、数字及下划线"));

$.validator.addMethod('alphanumeric', function(value, element, params) {
  return this.optional(element) || /^([a-zA-Z0-9_])*$/i.test(value);
}, jQuery.validator.format("请输入英文字母、数字及下划线"));

$.validator.addMethod('mobile', function(value, element, params) {
  return this.optional(element) || /^1\d{10}$/i.test(value);
}, jQuery.validator.format("请输入有效的手机号"));

$.validator.addMethod('byte_rangelength', function(value, element, params) {
...
}, jQuery.validator.format("请输输入{0}-{1}个字符,一个中文字算2个字符"));
